...
export const getLearningStates = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.subjectId) notesQuery = notesQuery.eq("subject_id", data.subjectId);
    ...
    const [proficiencyResult, eventResult, sessionsResult] = await Promise.all([
      db.from("topic_proficiency").select("note_id,total_attempts,correct_attempts,last_reviewed").eq("user_id", context.userId).in("note_id", noteIds),
      db.from("learning_events").select("event_type,note_id,subject_id,occurred_at").eq("user_id", context.userId).in("note_id", noteIds).order("occurred_at", { ascending: false }),
      db.from("study_sessions").select("subject_id,completed,ended_at").eq("user_id", context.userId).eq("completed", true),
    ]);
